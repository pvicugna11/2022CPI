...
namespace FantomLib
{
    ...
    public static class XPlayerPrefs
    {
        ...
        /// <param name="key">保存キー</param>
        /// <param name="def">デフォルト値</param>
        /// <returns>保存されていたオブジェクト（新規に生成）</returns>
        ...
        /// <typeparam name="T">オブジェクトの型</typeparam>
        /// <param name="key">保存キー</param>
        /// <param name="obj">上書きするオブジェクト</param>
        ...
        /// <param name="key">保存キー</param>
        /// <param name="def">デフォルト値</param>
        /// <returns>保存されていた配列（新規に生成）</returns>
        ...
        /// <param name="key">保存キー</param>
        /// <param name="def">デフォルト値</param>
        /// <returns>保存されていたリスト（新規に生成）</returns>
        ...
        //静的配列をラップ
        //セーブ時：コンストラクタから 配列に代入 → JSON で保存（"array":[0,1,2,…]; のようになる）
        //ロード時：JSON から → 配列に上書き → 複製を返す
        [Serializable]
        ...
        //リストをラップ
        //セーブ時：コンストラクタから list に代入 → JSON で保存（"list":[0,1,2,…]; のようになる）
        //ロード時：JSON から → list に上書き → 複製を返す
        [Serializable]
        ...
        //ロード時：JSON から → keys, values 上書き → 辞書（または配列）に変換したものを返す
        //※キー＆値ペア配列は長さを同じにすること
        [Serializable]
        private class DictionaryWrap<K, V>
        {
            ...
            public DictionaryWrap(K[] keys, V[] values)     //※長さを合わせること
            {
                this.keys = keys;
                this.values = values;
            }
            ...
            public Dictionary<K, V> ToDictionary()
            {
                try {
                    return keys.Select((k, i) => new { k, v = values[i] })
                        .ToDictionary(a => a.k, a => a.v);      //※重複キーがあるとエラーとなる
                }
                catch {
                    return new Dictionary<K, V>();  //空の辞書
                }
            }
